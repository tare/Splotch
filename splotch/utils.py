    ...
    if not isinstance(variables_of_interest, list):
        ...
        for variable_of_interest in variables_of_interest:
            if not isinstance(variable_of_interest, list):
                ...
                output.append(
                    [
                        variables.index(variable_of_interest_tmp)
                        for variable_of_interest_tmp in variable_of_interest
                    ]
                )
        return output
...
def read_stan_csv(filename, variables):
    ...
    for variable in variables:
        ...
        if (
            len([foo for foo in list(data.columns) if foo.startswith(variable + ".")])
            == 0
        ):
            dimensions = [N_samples, 1]
        # non scalar parameter
        else:
            dimensions = numpy.hstack(
                (
                    N_samples,
                    numpy.array(
                        [
                            list(map(int, foo.split(".")[1:]))
                            for foo in list(data.columns)
                            if foo.startswith(variable)
                        ]
                    ).max(0),
                )
            )
    ...
    for col in data.columns:
        ...
        # non scalar parameter
            ...
            samples[variable][tuple([slice(0, N_samples)]) + tuple(indices)] = data[
                col
            ].values
...
def savagedickey(
    samples1, samples2, prior1_mean=0.0, prior1_std=2.0, prior2_mean=0.0, prior2_std=2.0
):
    ...
    numerator = scipy.stats.norm.pdf(
        0,
        loc=prior1_mean - prior2_mean,
        scale=numpy.sqrt(prior1_std**2 + prior2_std**2),
    )
...
def get_variable_mappings(count_files, metadata, levels, n_levels):
    conditions_to_variables = {
        "beta_level_%d" % (idx + 1): [] for idx in range(0, n_levels)
    }
    ...
    if n_levels == 3:
        ...
        for level_1_idx, level_1 in enumerate(levels_1, start=1):
            ...
            for level_2 in levels[level_1]:
                logging.info(
                    "beta_level_2[%d] := %s %s" % (level_2_idx, level_1, level_2)
                )
                conditions_to_variables["beta_level_2"].append(
                    "%s %s" % (level_1, level_2)
                )
                ...
                for level_3 in levels[level_1][level_2]:
                    logging.info(
                        "beta_level_3[%d] := %s %s %s"
                        % (level_3_idx, level_1, level_2, level_3)
                    )
                    conditions_to_variables["beta_level_3"].append(
                        "%s %s %s" % (level_1, level_2, level_3)
                    )
                    last_level_identifiers[
                        str([level_1, level_2, level_3])
                    ] = level_3_idx
...
def watershed_tissue_sections(unique_label, labels, max_label):
    ...
    while True:
        indices = peak_local_max(
            distance
            + 0.05 * numpy.random.rand(tmp_labels.shape[0], tmp_labels.shape[1]),
...
def print_summary(levels_list, coordinates_list):
    columns = ["Level %d" % (idx + 1) for idx in range(0, len(levels_list[0]))] + [
        "Number of spots"
    ]
    ...
    summary_df = pd.DataFrame(
        [
            list(levels) + [len(coordinates)]
            for levels, coordinates in zip(levels_list, coordinates_list)
        ],
        columns=columns,
    )
    ...
    if len(columns[0:-2]) > 0:
        logging.info(
            summary_df.groupby(columns[0:-2]).agg(
                {"Number of spots": ["size", "sum"], columns[-2]: "nunique"}
            )
        )
    else:
        logging.info(
            summary_df.groupby(columns[0]).agg({"Number of spots": ["size", "sum"]})
        )
...
def read_array(filename):
    ...
    array_coordinates_float = numpy.array(
        [
            list(map(float, coordinate.split("_")))
...
def read_array_metadata(metadata, filename, n_levels):
    ...
    array_levels = [
        array_metadata["Level %d" % (idx + 1)].values[0] for idx in range(0, n_levels)
    ]
...
def detect_tissue_sections(coordinates, check_overlap=False, threshold=120):
    ...
    unique_labels, unique_labels_counts = numpy.unique(
        labels * array, return_counts=True
    )
    ...
    if check_overlap:
        for unique_label, unique_label_counts in zip(
...
def get_spot_adjacency_matrix(coordinates):
    ...
    dist_matrix[
        numpy.logical_or(dist_matrix > 1.2, numpy.isclose(dist_matrix, 0))
    ] = numpy.inf
    W = 1.0 / dist_matrix
    W[W > 0] = 1
...
def get_counts(gene_idx, N_tissues, counts_list):
    ...
    for tissue_idx in range(0, N_tissues):
        concatenated_counts = concatenated_counts + list(
            counts_list[tissue_idx][:, gene_idx]
        )
...
def generate_column_labels(files_list, coordinates_list):
    filenames = [
        [files_list[r]] * len(coordinates_list[r])
...
def generate_dictionary(
    ...
    for tissue_idx in range(0, N_tissues):
        concatenated_size_factors = concatenated_size_factors + list(
            size_factors_list[tissue_idx]
        )
    data["size_factors"] = concatenated_size_factors

    concatenated_D = []
    for tissue_idx in range(0, N_tissues):
        concatenated_D = concatenated_D + [
            numpy.where(tissue_section_aar_matrix)[0][0] + 1
    ...
    if car:
        ...
        data["W_sparse"] = generate_W_sparse(
            sum(N_spots_list), data["W_n"][0], W.toarray()
        )
        data["D_sparse"] = W.sum(1).A1.astype(int)
        data["eig_values"] = numpy.linalg.eigvalsh(
            diags(1.0 / numpy.sqrt(data["D_sparse"]), 0, format="csr")
            .dot(W)
            .dot(diags(1.0 / numpy.sqrt(data["D_sparse"]), 0, format="csr"))
            .toarray()
        )
    else:
        data["W_n"] = []
...
def get_tissue_section_spots(
    tissue_idx, array_coordinates_float, spots_tissue_section_labeled
):
    ...
    for n, coord in enumerate(array_coordinates_float):
        if (
            spots_tissue_section_labeled[
                int(numpy.round(coord[0])), int(numpy.round(coord[1]))
            ]
            == tissue_idx
        ):
...
def filter_arrays(
    ...
    coordinates_float=None,
    counts=None,
    counts_per_spot=None,
    size_factors=None,
    aar_matrix=None,
    W=None,
):
...
def get_tissue_sections(
    count_files,
    metadata,
    minimum_sequencing_depth=100.0,
    maximum_number_of_spots_per_tissue_section=2000.0,
):
    data = {}
    ...
    for filename in count_files:
        ...
        tissue_section_labels, spots_tissue_section_labeled = detect_tissue_sections(
            array_coordinates_float, True, maximum_number_of_spots_per_tissue_section
        )
        ...
        for tissue_idx in tissue_section_labels:
            ...
            data[filename].append(
                {
                    "coordinates": tissue_section_coordinates_str,
                    "coordinates_num": numpy.asarray(
                        [
                            list(map(float, spot.split("_")))
                            for spot in tissue_section_coordinates_str
                        ]
                    ),
                    "annotations": numpy.asarray(
                        [
                            numpy.where(spot)[0][0]
                            for spot in tissue_section_aar_matrix.T
                        ]
                    ),
                }
            )
...
def registration_individuals(x, y, aar_names, max_iter=10000, aars=None):
    ...
    def cost_function(x, y):
        ...
        return sum(
            [
                foo(x[:, aar_indices[aar]], uti_indices[aar])
                for aar in range(0, len(aars))
            ]
        )
    ...
    def transform(param, x):
        ...
        return np.hstack(
            [
                ...
                + np.expand_dims(delta_p, 1)
                for theta, delta_p, x_s in zip(thetas, delta_ps.T, x)
            ]
        )
    ...
    net_params = numpy.hstack(
        (numpy.random.uniform(-numpy.pi, numpy.pi, len(x)), numpy.zeros(2 * len(x)))
    )
...
def registration(count_files, metadata, max_iter=10000, aars=None):
    ...
    coordinates_float = [
        ...
        for coordinates in coordinates_float
    ]
    logging.info("Aligning the tissue sections")
    coordinates_float = registration_individuals(
        coordinates_float, annotations, aar_names, max_iter, aars
    )
    logging.info("Rotating the consensus spot cloud")
    coordinates_float = registration_consensus(
        coordinates_float, annotations, aar_names
    )
    ...
    for idx in range(0, coordinates_float.shape[1]):
        ...
        registered_coordinates[array_filenames[idx]][coordinates_str[idx]] = "_".join(
            map(str, coordinates_float[:, idx])
        )
