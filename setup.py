...
def compile_stan_code():
    ...
    url_template = (
        "https://github.com/stan-dev/cmdstan/releases/download/%s/cmdstan-%s.tar.gz"
    )
    filename, _ = urllib.request.urlretrieve(
        url_template % (latest_version, latest_version.replace("v", ""))
    )
    ...
    proc = subprocess.run(
        ["make", "build"],
        cwd=os.path.join(
            temp_dir.name, "cmdstan-%s" % (latest_version.replace("v", ""))
        ),
        stdout=sys.stdout,
        stderr=sys.stderr,
    )
...
stan_files = [
    re.sub(".stan$", "", stan_file)
    for stan_file in glob.glob(os.path.join(os.getcwd(), "stan", "*.stan"))
]
...
setup(
    ...
    packages=["splotch"],
    scripts=[
        "bin/splotch_prepare_count_files",
        "bin/splotch_generate_input_files",
        "bin/splotch",
    ],
    # could not pass binaries through the scripts argument
    data_files=[("bin", stan_files)],
    install_requires=install_requires,
    cmdclass={"install": InstallCommand, "install_data": StanCodeCompilation},
)
